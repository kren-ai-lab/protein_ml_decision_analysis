...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Dimensionality reduction and cosine similarity analysis for protein embeddings (training only)."
    )

    parser.add_argument("--emb-train", required=True, help="CSV with training embeddings.")
    ...
    parser.add_argument(
        ...
        help="Filename for saved training embedding matrix (.npy)."
    )
...
def plot_embeddings_1x3(df_pca, df_tsne, df_umap, label_col, output_path: Path, show=False):
    ...
    sns.scatterplot(
        data=df_pca, x="pca_1", y="pca_2", hue=label_col,
        palette="Set2", alpha=0.8, s=40, ax=axes[0]
    )
    axes[0].set_title("PCA")
    axes[0].legend(loc="best")

    sns.scatterplot(
        data=df_tsne, x="tsne_1", y="tsne_2", hue=label_col,
        palette="Set2", alpha=0.8, s=40, ax=axes[1], legend=False
    )
    axes[1].set_title("t-SNE")

    sns.scatterplot(
        data=df_umap, x="umap_1", y="umap_2", hue=label_col,
        palette="Set2", alpha=0.8, s=40, ax=axes[2], legend=False
    )
...
def main():
    ...
    # Load input
    # -----------------------------
    df_embedding_train = pd.read_csv(args.emb_train)
    ...
    # ordenar features por índice numérico: p_0, p_1, ..., p_319
    ...
    try:
        ...
        raise ValueError(
            ...
            "Please ensure columns with prefix 'p_' are numeric."
        ) from e
    ...
    # Save embeddings
    # -----------------------------
    np.save(out["artifacts"] / args.embedding_npy_output, X)
    ...
    summary_stats_df = pd.DataFrame({
        ...
        "value": [X.min(), X.max(), X.mean(), X.std(), X.shape[0], X.shape[1]]
    })
